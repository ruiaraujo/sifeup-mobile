...
public class AnalyticsUtils {
	...
	private AnalyticsUtils(final Context context) {
		...
		tpe.submit(new Runnable() {
			...
			public void run() {
				...
				// Since visitor CV's should only be declared the first time an
				// app runs, check if
				...
				if (firstRun) {
					...
					mTracker.setCustomVar(1, "apiLevel", apiLevel,
							VISITOR_SCOPE);
					mTracker.setCustomVar(2, "model", model, VISITOR_SCOPE);

					// Close out so we never run this block again, unless app is
					// removed & =
					// reinstalled.
		...
		tpe.submit(new Runnable() {
			...
			public void run() {
				try {
					...
					Log.d(TAG, "iosched Analytics trackEvent: " + category
							+ " / " + action + " / " + label + " / " + value);
				} catch (Exception e) {
		...
		tpe.submit(new Runnable() {
			...
			public void run() {
				try {
					...
					// exception.
					Log.w(TAG,
							"iosched Analytics trackPageView error: " + path, e);
				}
			}
		});
	}
	 ...
	 * available.
	 */
	private static AnalyticsUtils sEmptyAnalyticsUtils = new AnalyticsUtils(
			null) {
		@Override
