...
public class ContactOperations {
	...
	public ContactOperations addAltEmail(String email) {
		mValues.clear();
		if (!TextUtils.isEmpty(email)) {
			mValues.put(Email.DATA, email);
	...
	public ContactOperations addAvatar(ContentResolver resolver, String userCode) {
		if (userCode != null) {
			byte[] avatarBuffer = getProfilePic(resolver, userCode);
	...
	public ContactOperations updateAvatar(ContentResolver resolver,
			String userCode, Uri uri) {
		if (userCode != null) {
			byte[] avatarBuffer = getProfilePic(resolver, userCode);
			if (avatarBuffer != null) {
				mValues.clear();
	...
	private byte[] getProfilePic(ContentResolver resolver, String userCode) {
		...
		try {
			if (cursor.moveToFirst()) {
				final String path = cursor.getString(cursor
						.getColumnIndex(SigarraContract.Profiles.PIC));
