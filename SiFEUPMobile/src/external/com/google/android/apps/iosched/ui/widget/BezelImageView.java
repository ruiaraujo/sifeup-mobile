 ...
 * drawable on top. This is useful for applying a beveled look to image
...
public class BezelImageView extends ImageView {
	...
	public BezelImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		if (mBorderDrawable == null) {
			mBorderDrawable = getResources().getDrawable(
	...
	private void invalidateCache() {
		...
		mCached = Bitmap.createBitmap(mBounds.width(), mBounds.height(),
		...
		// certain drawables invalidate themselves on draw, e.g.
		// TransitionDrawable sets its alpha
		// which invalidates itself. to prevent stack overflow errors, we must
		// guard the
		// invalidation (see specialized behavior when guarded in invalidate()
		// below).
